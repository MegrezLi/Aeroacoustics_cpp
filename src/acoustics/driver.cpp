...
std::vector<std::string> lines(const std::string &path) {
    ...
    require(bool(stream), "Cannot read input file");
    std::vector<std::string> output;
    std::string line;
    while (std::getline(stream, line))
        output.push_back(line);
    if (!output.empty() && output[0].compare(0, 3, "\xef\xbb\xbf") == 0)
        output[0].erase(0, 3);
    return output;
}
...
void increasing(const Spectrum &v) {
    ...
    for (std::size_t i = 0; i < v.size(); ++i)
        require(std::isfinite(v[i]) && (i == 0 || v[i] > v[i - 1]),
...
BLTable BLTable::read(const std::string &path) {
    auto text = lines(path);
    require(text.size() >= 4, "Truncated BL table");
    ...
    for (int r = 0; r < nr; ++r) {
        while (pos < text.size() && (text[pos].find_first_not_of(" \t\r") == std::string::npos ||
                                     text[pos][text[pos].find_first_not_of(" \t\r")] == '!'))
            ++pos;
        require(pos + 3 + na <= text.size(), "Truncated BL rows");
        result.reynolds.push_back(std::stod(text[pos]) * 1e6);
        pos += 3;
        for (int a = 0; a < na; ++a) {
            std::istringstream row(text[pos++]);
            double angle;
            std::array<double, 8> values{};
            require(bool(row >> angle) && std::isfinite(angle), "Invalid BL AoA");
            for (auto &v : values)
                require(bool(row >> v) && std::isfinite(v), "Invalid BL value");
            if (r == 0) {
                raw_aoa.push_back(angle);
                result.aoa.push_back(angle - 360 * std::floor((angle + 180) / 360));
            } else
                require(std::abs(raw_aoa[a] - angle) <= 1e-8 + 1e-5 * std::abs(raw_aoa[a]),
                        "BL AoA grids differ");
            result.values.push_back(values);
        }
    }
    increasing(result.aoa);
...
std::pair<std::size_t, Spectrum> blade_elements(const Spectrum &span, double percent) {
    increasing(span);
    require(span.front() >= 0 && percent > 0 && percent <= 100, "Invalid span or percentage");
    const auto n = span.size();
    std::size_t start = std::max<std::size_t>(1, n - 1);
    ...
    for (auto j = start; j < n; ++j)
        length[j] = j == 0 ? span[j] : j == n - 1 ? span[j] - span[j - 1] : (span[j + 1] - span[j - 1]) / 2;
    return {start, length};
}
std::array<double, 2> guidati_thickness(const std::vector<std::array<double, 2>> &coords) {
    ...
    for (std::size_t j = 2; j < coords.size(); ++j)
    ...
    for (double target : {.01, .10}) {
        ...
        for (std::size_t j = 1; j < split; ++j)
            if (std::abs(coords[j][0] - target) < std::abs(coords[upper][0] - target))
                upper = j;
        for (std::size_t j = split + 1; j < coords.size(); ++j)
            if (std::abs(coords[j][0] - target) < std::abs(coords[lower][0] - target))
                lower = j;
        output[k++] = coords[lower][1] - coords[upper][1];
    }
    return output;
}
TurbulenceState::TurbulenceState(Spectrum span, std::size_t blades, double dt, double height, int method,
                                 double ti, double avgv)
    : span_(std::move(span)), blades_(blades), height_(height), ti_(ti), avgv_(avgv), method_(method) {
    increasing(span_);
    require(span_[0] >= 0 && blades > 0 && dt > 0 && (method == 1 || method == 2),
            "Invalid TI initialization");
    samples_ = std::max<std::size_t>(std::floor(5 / dt + .5), 1);
    auto regions = static_cast<std::size_t>(std::ceil(span_.back() / 5)) + 1;
    for (double r : span_)
        radial_.push_back(std::clamp<std::size_t>(std::ceil(r / 5), 1, regions) - 1);
    buffers_.assign(regions * 6 * samples_, 0);
    counts_.assign(regions * 6, 0);
    values.assign(blades * span_.size(), 0);
}
void TurbulenceState::update(const Spectrum &vrel, const std::vector<Vec3> &inflow,
                             const std::vector<Vec3> &leading) {
    require(vrel.size() == values.size() && inflow.size() == values.size() && leading.size() == values.size(),
            "TI array shape mismatch");
    for (std::size_t k = 0; k < values.size(); ++k) {
        if (method_ == 1) {
            require(vrel[k] != 0, "TI scaling requires nonzero Vrel");
            values[k] = ti_ * avgv_ / vrel[k];
            continue;
        }
        double z = leading[k][2] - height_;
        std::size_t angular = 0;
        if (std::abs(z) > std::max(std::abs(z), 1.) * std::numeric_limits<double>::epsilon() * 50) {
            double angle = std::atan2(leading[k][1], z) * 180 / pi;
            angle -= 360 * std::floor(angle / 360);
        ...
        if (count <= samples_) {
            ...
            for (std::size_t j = 0; j < samples_; ++j)
                variance += std::pow(begin[j] - mean, 2);
            values[k] =
                std::abs(mean) > std::max(std::abs(mean), 1.) * std::numeric_limits<double>::epsilon() * 50
                    ? std::sqrt(variance / samples_) / mean
                    : 0.;
        }
    }
}
Snapshot snapshot_spectrum(const Parameters &p, const std::vector<Node> &nodes,
                           const std::vector<Vec3> &observers) {
    AcousticWorkspace workspace(p);
    return workspace.evaluate(nodes, observers);
}
...
const Snapshot *AcousticDriver::step_view(double time, const std::vector<std::vector<Node>> &blades) {
    ...
    for (std::size_t b = 0; b < blades_; ++b) {
        ...
        for (std::size_t j = 0; j < span_.size(); ++j) {
            const auto &node = blades[b][j];
            const auto k = b * span_.size() + j;
            speeds_[k] = node.section.speed;
            inflow_[k] = node.inflow;
            auto &position = leading_[k];
            position = node.aero_center;
            for (int axis = 0; axis < 3; ++axis)
                position[axis] +=
                    node.section.chord * (-node.airfoil_reference[1] * node.global_to_local[axis] -
                                          node.airfoil_reference[0] * node.global_to_local[3 + axis]);
...
std::optional<Snapshot> AcousticDriver::step(double time, const std::vector<std::vector<Node>> &blades) {
    const auto *result = step_view(time, blades);
    return result ? std::optional<Snapshot>(*result) : std::nullopt;
}
std::vector<Vec3> read_observers(const std::string &path) {
    auto text = lines(path);
    require(text.size() >= 3, "Truncated observer file");
    int n = std::stoi(text[0]);
    require(n > 0 && text.size() >= std::size_t(n + 2), "Invalid observer count");
    std::vector<Vec3> result(n);
    for (int i = 0; i < n; ++i) {
        std::istringstream row(text[i + 2]);
        for (auto &v : result[i])
            require(bool(row >> v) && std::isfinite(v), "Invalid observer position");
    }
    return result;
}
std::pair<Parameters, std::map<std::string, std::string>> read_aa_input(const std::string &path) {
    ...
    for (auto [key, member] :
         std::vector<std::pair<std::string, double Parameters::*>>{{"spdsound", &Parameters::spdsound},
                                                                   ...
                                                                   {"airdens", &Parameters::airdens},
                                                                   {"lturb", &Parameters::lturb},
                                                                   {"alprat", &Parameters::alprat},
                                                                   {"ti", &Parameters::ti},
                                                                   {"avgv", &Parameters::avgv}})
        if (fields.count(key))
            p.*member = std::stod(fields[key]);
    for (auto [key, member] :
         std::vector<std::pair<std::string, int Parameters::*>>{{"blmod", &Parameters::x_blmethod},
                                                                {"tripmod", &Parameters::itrip},
                                                                {"timod", &Parameters::timod},
                                                                {"tbltemod", &Parameters::tbltemod},
                                                                {"lammod", &Parameters::lammod},
                                                                {"tipmod", &Parameters::tipmod},
                                                                {"bluntmod", &Parameters::bluntmod}})
        if (fields.count(key))
            p.*member = std::stoi(fields[key]);
    for (auto [key, member] : std::vector<std::pair<std::string, bool Parameters::*>>{
             {"roundedtip", &Parameters::round}, {"aweighting", &Parameters::aweighting}})
        if (fields.count(key)) {
            auto v = lower(fields[key]);
            require(v == "true" || v == "false", "Invalid boolean in AA input");
            p.*member = v == "true";
        }
    return {p, fields};
}
