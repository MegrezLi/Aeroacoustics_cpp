...
double detail::integrate_tno(double omega, double lower, double upper, bool suction, double mach,
    ...
    auto &e1 = work.exp_gauss, &e2 = work.exp_decay;
    exponential(gauss, e1);
    exponential(decay, e2);
    for (std::size_t i = 0; i < factor.size(); ++i)
        factor[i] *= e1[i] * e2[i];
    auto &pressure = work.pressure;
    std::fill(pressure.begin(), pressure.end(), 0.);
#ifdef AERO_USE_MKL
    cblas_dgemv(CblasRowMajor, CblasNoTrans, 61, 61, 1., factor.data(), 61, weights.data(), 1, 0.,
                pressure.data(), 1);
#else
    for (int i = 0; i < 61; ++i)
        for (int j = 0; j < 61; ++j)
            pressure[i] += factor[i * 61 + j] * weights[j];
#endif
    for (int i = 0; i < 61; ++i)
...
double spl_integrate(double omega, double lower, double upper, bool suction, double mach,
    ...
    return detail::integrate_tno(omega, lower, upper, suction, mach, bl, p, work);
}
std::pair<Spectrum, Spectrum> tblte_tno(double speed, double theta, double phi, double span, double distance,
    ...
    s.bl = bl;
    detail::TnoWorkspace work;
    detail::TnoSource source;
    detail::prepare_tno(p, s, work, source);
    Spectrum pressure, suction;
    detail::emit_tno(source, {distance, theta, phi}, pressure, suction);
    return {std::move(pressure), std::move(suction)};
}
} // namespace aeroacoustics
